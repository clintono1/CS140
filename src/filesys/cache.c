...
void
cache_read_hit(block_sector_t sector, void *buffer, off_t start,
...
cache_read_miss(block_sector_t sector, void *buffer, off_t start, off_t length)
{
  ...
  if(cache_used_num < BUFFER_CACHE_SIZE)
  {
	...
	cond_signal(&buffer_cache[cache_id].can_write, buffer_cache[cache_id].lock);
  ...
  else
  {
	...
	while(1)
	{
	  ...
	  else
	  {
		...
		lock_release(&global_cache_lock);
		block_sector_t old_sector = buffer_cache[evict_id];
		                ...
		                off_t start, off_t length)
{
  ...
  uint32_t cache_id_hit = is_in_cache(sector);
  if(cache_id_hit != -1)
  {
	cache_read_hit(sector, buffer, start, length, cache_id_hit);
  }
  else
  {
